...
我们的设计为执行各种任务的许多并发读取器和写入器提供了高吞吐量。 我们通过将通过master的文件系统控制与直接在chunkserver和客户端之间传递的数据传输分开来实现此目的。 大型chunk的大小和chunk的租用将master操作对常见操作的影响降到最低，这将权限授予数据修改中的主副本。 这使得不会成为瓶颈的简单、集中的主服务器成为可能。 我们认为，网络堆栈的改进将解除当前对单个客户端看到的写入吞吐量的限制。
